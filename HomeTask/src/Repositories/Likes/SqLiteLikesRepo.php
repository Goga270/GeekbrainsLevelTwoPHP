...
class SqLiteLikesRepo implements LikesRepositoryInterface
{
    ...
    public function save(Like $like): void
    {
        ...
        $statement = $this->connection->prepare(
            'INSERT INTO likes (uuid, articleUuid, userUuid)
    ...
    public function getAllByArticle(UUID $id): iterable
    {
        ...
        $statement = $this->connection->prepare(
            'SELECT * FROM likes WHERE articleUuid = :uuid'
        );
        $statement->execute([
            ':uuid' => (string)$id,
        ]);
    ...
    private function getAllLikes(PDOStatement $statement): iterable
    {
        ...
        while ($result){
            $likes [] = new Like(new UUID($result['uuid']), new UUID($result['articleUuid']), new UUID($result['userUuid']));
            $result = $statement->fetch(PDO::FETCH_ASSOC);
        }
